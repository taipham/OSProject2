...
// DONE
MODULE_AUTHOR("Tai Pham & Daifu Ye");
...
static void osprd_process_request(osprd_info_t *d, struct request *req)
{
	...
	//eprintk("Process request...\n");
	if (req->current_nr_sectors > nsectors)
	{
		eprintk("nr_sectors too big\n");
		end_request(req,0);
	}
	if (req->sector + req->current_nr_sectors > nsectors)
	{
		eprintk("Request undefined sectors\n");
		end_request(req, 0);
	}
...
static int osprd_close_last(struct inode *inode, struct file *filp)
{
	if (filp) {
		...
		// Your code here.
		// TODO: DONE ?
		if (d == 0) // file no correspond to OSP ramdisk
		{
			eprintk("osprd_info is NULL\n");
			return -1;
		}
