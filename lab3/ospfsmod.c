...
add_block(ospfs_inode_t *oi)
{
  ...
  int32_t in2idx = indir2_index(n);
	/* EXERCISE: Your code here */
	// exceed max num block
	if (n > OSPFS_MAXFILEBLKS)
	{
		return -EIO;
	}	

	// check if needed to add an indirect block
	...
	// check if need indirect^2 block
...
remove_block(ospfs_inode_t *oi)
{
    ...
    //
    int32_t di = direct_index(n-1);
    int32_t inidx = indir_index(n-1);
    int32_t in2idx = indir2_index(n-1);
    ...
    //
        ...
        block2 = ospfs_block(oi->oi_indirect2);
				block = ospfs_block(block2[inidx]);
				/*
        // check the error
        if (block2[inidx] == 0) {
            return -EIO;
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		n = 0;
		uint32_t curr_pos;
		curr_pos = *f_pos % OSPFS_BLKSIZE; // current position in block
		data += curr_pos; // start position
		
		n = OSPFS_BLKSIZE - curr_pos; // data left in this block
		if (n > count - amount) // end of file come before end of block
			n = count - amount;
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
  ...
  //char *block_buffer; // doesn't need this, char* data already provided
	uint32_t amount = 0;
    //int grow = 0;
	...
	while (amount < count && retval >= 0) {
		...
		/* EXERCISE: Your code here */
    //block_buffer = ospfs_block(blockno); data above provided
        //update block offset
		n = 0;
